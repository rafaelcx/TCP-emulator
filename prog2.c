...
void A_output(struct msg message) {
    ...
    // Setting up internal control information
    a_last_pkt = packet;
    a_state = 1;

    printf("\tSending Side: Sending packet: %s\n", message.data);
...
void A_input(struct pkt packet) {
    ...
    // Packet is corrupted, must ask for retry
    if (packet.checksum != get_checksum(&packet)) {
        printf("\tSending Side: Corrupted packet.\n");
        return;
    }

    // Ack received is from another packet, must ask for retry
    if (packet.acknum != a_seq) {
        printf("\tSending Side: Wrong Ack, probably from another packet.\n");
        return;
    }

    // Good looking Ack
    printf("\tSending Side: Got an Ack.\n");
    stoptimer(0);

    // Setting a_state as Nack to avoid duplicates
    a_seq = 1 - a_seq;
    a_state = 0;
}
...
void A_timerinterrupt(void) {
    ...
    printf("\tReceiving Side (timeinterrupt): Retrying last packet: %s\n", a_last_pkt.payload);
    tolayer3(0, a_last_pkt);
    starttimer(0, a_rtt);
}
...
void B_input(struct pkt packet) {
    ...
    if (packet.checksum != get_checksum(&packet)) {
        printf("\tReceiving Side: Corrupted packet, sending Nack\n");
        send_ack(1, 1 - b_seq);
        return;
    }

    // Sends a Nack because received packet was not being expected
    if (packet.seqnum != b_seq) {
        printf("\tReceiving Side: Wrong sequence number, sending Nack\n");
        send_ack(1, 1 - b_seq);
        return;
    }

    printf("\tReceiving Side: %s\n", packet.payload);
    printf("\tReceiving Side: Sending ACK\n");
    send_ack(1, b_seq);
    tolayer5(1, packet.payload);

    // Setting a_seq as Nack to avoid duplicates
    b_seq = 1 - b_seq;
}
